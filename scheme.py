...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Not the right amount of arguments')
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        answer = scheme_eval(expressions.rest.first, env)
        env.define(target, answer)
        return target
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(check, 2, 2)
        form = check.first
        arg = scheme_eval(check.rest.first, env)
        arguments = Pair(arg, arguments)
        formals = Pair(form, formals)
        bindings = bindings.rest
    check_formals(formals)
    return env.make_child_frame(formals, arguments)
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    def make_call_frame(self, args, env):
        next_frame = env.make_child_frame(self.formals, args)
        return next_frame
    # END PROBLEM 15
